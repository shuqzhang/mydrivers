...
// Final device, create private copy of device for each access
// data structure
struct scull_listitem
{
    struct scull_dev dev;
    struct list_head scull_list;
    dev_t key;
};
// list for all devices and the lock for securely access data
static LIST_HEAD(scull_c_list);
static DEFINE_SPINLOCK(scull_c_lock);

// A placehoder device which really hold the cdev stuff in driver
static struct scull_dev scull_c_dev;

// Lookup device, if device doesn't exist, create a new one
static struct scull_dev* scull_c_lookfor_device(dev_t key)
{
    ...
    lptr->key = key;
    list_add(&lptr->scull_list, &scull_c_list);
    return &(lptr->dev);
}
...
{
    // no action, device cannot be destroyed here, keep it persistent util driver cleanup
    return 0;
}

static struct file_operations scull_c_fops = {
    .owner = THIS_MODULE,
    .llseek = no_llseek, // TODO
    .read = scull_read, // TODO
    .write = scull_write, // TODO
    .unlocked_ioctl = scull_ioctl, // TODO
    .open = scull_c_open,
...
static struct scull_adev_info
{
    ...
    {"scull_copy", &scull_c_dev, &scull_c_fops}
};
